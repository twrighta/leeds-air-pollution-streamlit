...
# tooltip explainer for the pollution score - Max score 28 min is 7
...
def generate_predictions(_model, model_inputs):
    ...
    :param model_inputs: numpy array of input featurds
    ...
    rounded = np.round(predictions, 0)
    return rounded
...
def create_model_input(current_month, current_day, current_hour, air_temperature, absolute_humidity,
    ...
    :param _scaler: trained scaler (already loaded). Underscored so Streamlit does not try to hash it.
...
st.divider()  # horizontal dividing line

# Image of leeds:
st.image(
    "C:/Users/tomwr/Datascience/data_visualization/woodland_trust_natures_calendar/leeds_air_quality/assets/leeds_skyline.jpg",
    width=500)
...
st.write_stream(stream_heading("And we also need your current weather conditions"))
...
# These work but need in a button:
# all_preds = generate_predictions(model, model_input)


# Create a session state to store the predictions in. First have to initialize it
...
if generate_pred_btn:
    ...
    with hr12:
        ...
        st.text(str(int(preds_df["12hr Forecast"].values)), help=pollution_tooltip(int(preds_df["12hr Forecast"].values)))
    with hr24:
        st.write(f"At {calc_future_time(hour_input, 24)}")
        st.text(str(int(preds_df["24hr Forecast"].values)), help=pollution_tooltip(int(preds_df["24hr Forecast"].values)))
